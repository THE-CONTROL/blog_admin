<template>
  <div class="w-full text-slate-900 border-t-2">
        ...
        w-10/12 sm:w-6/12 pb-4 mt-5">
            ...
            <div class="w-full flex justify-center">
                <input class="w-11/12 rounded-md bg-slate-100 focus:bg-slate-200
                text-sm p-1" v-model="pronouns" placeholder="What are your pronouns?">
            </div>
...
export default {
    ...
    setup( props ) {
        ...
        function update() {
            fetch("https://osamamwen.pythonanywhere.com/admin/update", {
                method: "PUT",
                headers: {
                    "Content-Type":"application/json",
                    Authorization: `Bearer ${localStorage.getItem('admin-access')}`
                },
                body: JSON.stringify({admin_name: username.value, picture: picture.value, about: about.value,
